...
from baselines.online_eval import batch_search, graph_search, batch_web_search, sample_response
...
def parse_args():
    ...
    parser.add_argument('--aux_model_name', type=str, default="Qwen2.5-32B-Instruct",
    ...
    parser.add_argument('--aux_tokenizer_path', type=str, default="/share/project/llm/Qwen2.5-32B-Instruct",
        ...
        try:
            async with semaphore:
                if generate_mode == "chat":
                    ...
                    if (
                            'deepseek' in model_name.lower() or 'r1' in model_name.lower()) and "<think>\n" not in formatted_prompt:
            ...
            loguru.logger.info(f"Generate Response Error occurred: {e}, Starting retry attempt {attempt + 1}")
            # loguru.logger.info(prompt)
            if "maximum context length" in str(e).lower():
                # If length exceeds limit, reduce max_tokens by half
                max_tokens = max_tokens // 2
                loguru.logger.info(f"Reducing max_tokens to {max_tokens}")
            if attempt == retry_limit - 1:
                loguru.logger.info(f"Failed after {retry_limit} attempts: {e}")
                return "", ""
...
async def generate_deep_web_explorer(
    ...
    local_search_results, web_search_results = [], []
    ...
    while True:
        ...
        if response.rstrip().endswith(END_SEARCH_QUERY):
            ...
            if new_query is None or END_SEARCH_QUERY in new_query or len(
                    new_query) <= 5 or new_query in invalid_search_queries:
                continue
            if new_query:
                ...
                #             results = await bing_web_search_async(new_query, args.bing_subscription_key,
                #                                                   args.bing_endpoint)
                ...
                #     except Exception as e:
                #         loguru.logger.info(f"Error during search query '{new_query}' using {args.search_engine}: {e}")
                #         results = {}
                # loguru.logger.info(f'- Searched for "{new_query}" using {args.search_engine}')
                #
                # if args.search_engine == "bing":
                ...
                results = await asyncio.to_thread(web_search_with_retry, args.web_retrieval_url, search_query, top_n=args.top_k)
                ...
                if args.sources == "all":
                    ...
                    graph_search_info = graph_search(remote_retriever_url, search_query, top_n=5)
                    local_search_info = f"\nLocal Search Results:\n{chunk_search_info}\nGraph Search Results:\n{graph_search_info}"

                    formatted_documents += local_search_info
                    local_search_results.append(chunk_search_info)
        ...
        # Check for click link
            ...
            if url and click_intent:
                ...
                loguru.logger.info(f"- Clicking on URL: {url} with intent: {click_intent}")
                ...
                if url not in url_cache:
                    try:
                        ...
                        loguru.logger.info(f"Error fetching URL {url}: {e}")
                        content = ""
                else:
    ...
    return output, original_prompt, local_search_results, web_search_results
...
async def process_single_sequence(
    ...
    while not seq['finished']:
        ...
        if seq['search_count'] < args.max_search_limit and total_tokens < MAX_TOKENS:
            if search_query is None or len(
                    search_query) <= 5 or END_SEARCH_QUERY in search_query or search_query in invalid_search_queries:  # 不合法的query
                continue
            ...
            #     try:
            #         if args.search_engine == "bing":
            #             results = await bing_web_search_async(search_query, args.bing_subscription_key,
            #                                                   args.bing_endpoint)
            #         elif args.search_engine == "serper":
            #             results = await google_serper_search_async(search_query, args.serper_api_key)
            #         elif args.search_engine == "original":
            #             results = batch_web_search(args.web_retrieval_url, search_query, "default", top_n=10)
            #         elif args.search_engine == "all":
            #             results = batch_web_search(args.web_retrieval_url, search_query, "websearch", top_n=10)
            #         else:  # Should not happen
            #             results = {}
            #         search_cache[search_query] = results
            #     except Exception as e:
            #         loguru.logger.info(f"Error during search query '{search_query}' using {args.search_engine}: {e}")
            #         results = {}
            # loguru.logger.info(f'Searched for: "{search_query}" using {args.search_engine}')
            #
            # if args.search_engine == "bing":
            #     relevant_info = extract_relevant_info(results)[:args.top_k]
            # elif args.search_engine == "serper":
                ...
                try:
                    ...
                    for url, content in contents.items():
                        ...
                        if not has_error:
                            url_cache[url] = content
                        # else:
                        #     loguru.logger.info(f'---Fetching Error\n{content}')
                except Exception as e:
                    loguru.logger.info(f"Error fetching URLs: {e}")
            ...
            analysis, explorer_prompt, local_search_explorer, web_search_explorer = await generate_deep_web_explorer(
                client=client,
            ...
            seq["local_search_results"].extend(local_search_explorer)
            seq["web_search_results"].extend(web_search_explorer)
...
async def main_async():
    ...
    for dataset_name in dataset_names:
        ...
        web_explorer = [seq['web_explorer'] for seq in rollout_sequences]
        ...
        with open(save_path, 'w') as f:
            for i, item in enumerate(filtered_data):
                ...
                item['web_explorer'] = web_explorer[i]
