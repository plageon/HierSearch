...
from verl.utils.torch_functional import get_response_mask, pad_sequence_to_length, get_eos_mask
...
class vLLMRolloutWithSearch(vLLMRollout):
    def __init__(self, actor_module: nn.Module, config: DictConfig, tokenizer, model_hf_config, **kwargs):
        ...
        self.start_patterns = ['<search>', '<graph_search>', '<get_adjacent_passages>', '<browse_url>']
        self.end_patterns = ['</search>', '</graph_search>', '</get_adjacent_passages>', '</browse_url>']

    @retry(max=5, sleep=1)
    def webpage_browse(self, query: Union[str, List[str]], browse_url: Union[str, List[str]], top_n=5) -> List[str]:
        if len(query) == 0:
        ...
        data = {'query': query, 'url': browse_url, 'top_n': top_n, }
    ...
    def batch_search(self, query: Union[str, List[str]], top_n=5) -> List[str]:
        ...
        # perform websearch simultaneously
        websearch_url = f"{self.config.search_url}/batch_web_search"
        websearch_data = {'query': query, 'top_n': top_n, }
        websearch_response = requests.post(websearch_url, json=websearch_data)
        ...
        websearch_result_list = []
        for item in websearch_response.json():
            curr_result = ''
            for line in item:
                curr_result += f"{line['contents']}\n\n"
            websearch_result_list.append(curr_result.strip())

        # merge the result
        for i in range(len(result_list)):
            result_list[i] += websearch_result_list[i]
                    ...
                    for start_pattern, end_pattern in zip(self.start_patterns, self.end_patterns):
                        if len(search_queries[start_pattern]) > 0:
                            ...
                            if start_pattern == '<search>':
                                ...
                                query, browse_url, browse_idx = [], [], []
                                for i in range(len(search_queries[start_pattern])):
                                    item = search_queries[start_pattern][i]
                                    try:
                                        split_item = item.split(',')
                                        query.append(split_item[0].strip())
                                        browse_url.append(split_item[1].strip())
                                        browse_idx.append(i)
                                    except:
                                        loguru.logger.warning(f"malformed query: {item}")

                                valid_search_results = self.webpage_browse(query, browse_url)
                                ...
                                for i in range(len(search_queries[start_pattern])):
                                    if i in browse_idx:
                                        ...
                                        search_results.append("invalid url")
                            else:
