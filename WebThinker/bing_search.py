    ...
    try:
        if use_jina:
            ...
            try:
                ...
                has_error = (any(indicator.lower() in response.text.lower() for indicator in error_indicators) and len(
                    response.text.split()) < 64) or response.text == ''
                if has_error:
...
def fetch_page_content(urls, max_workers=32, use_jina=False, jina_api_key=None, snippets: Optional[dict] = None,
                       show_progress=False, keep_links=False):
    """
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(extract_text_from_url, url, use_jina, jina_api_key, snippets.get(url) if snippets else None,
                            keep_links): url
            for url in urls
        }
    ...
    try:
        if use_jina:
            ...
            async with session.get(url) as response:
                ...
                # 检查是否有错误指示
                has_error = (any(indicator.lower() in html.lower() for indicator in error_indicators) and len(
                    html.split()) < 64) or len(html) < 50 or len(html.split()) < 20
