...
class HippoRAG:
    ...
    def __init__(self,
        ...
        self.language = self.global_config.language

        self.prompt_template_manager = PromptTemplateManager(role_mapping={"system": "system", "user": "user", "assistant": "assistant"}, language=self.language)
    ...
    def index(self, docs: List[str]):
        ...
        loguru.logger.info(f"Indexing Documents")

        loguru.logger.info(f"Performing OpenIE")
        ...
        loguru.logger.info(f"Loaded {len(chunk_to_rows)} chunks")

        all_openie_info, chunk_keys_to_process = self.load_existing_openie(chunk_to_rows.keys())
        new_openie_rows = {k : chunk_to_rows[k] for k in chunk_keys_to_process}

        if len(chunk_keys_to_process) > 0:
            loguru.logger.info(f"Performing OpenIE for {len(new_openie_rows)} new chunks.")
            new_ner_results_dict, new_triple_results_dict = self.openie.batch_openie(new_openie_rows)
            self.merge_openie_results(all_openie_info, new_openie_rows, new_ner_results_dict, new_triple_results_dict)
        ...
        loguru.logger.info(f"Encoding Entities")
        self.entity_embedding_store.insert_strings(entity_nodes)
        loguru.logger.info(f"Loaded {len(entity_nodes)} entity nodes")

        loguru.logger.info(f"Encoding Facts")
        # loguru.logger.info(f"{facts[1000:1010]}")
        self.fact_embedding_store.insert_strings([str(fact) for fact in facts])
        loguru.logger.info(f"Loaded {len(facts)} facts")

        loguru.logger.info(f"Constructing Graph")

        self.node_to_node_stats = {}
        self.ent_node_to_chunk_ids = {}

        entity_node_to_chunk_ids_save_path = os.path.join(self.working_dir, "ent_node_to_chunk_ids.json")
        if os.path.exists(entity_node_to_chunk_ids_save_path):
            with open(entity_node_to_chunk_ids_save_path, "r") as f:
                self.ent_node_to_chunk_ids = json.load(f)
                self.ent_node_to_chunk_ids = {k: set(v) for k, v in self.ent_node_to_chunk_ids.items()}
            loguru.logger.info(f"Loaded {len(self.ent_node_to_chunk_ids)} entity nodes to chunk ids from {entity_node_to_chunk_ids_save_path}")
        else:
            self.add_fact_edges(chunk_ids, chunk_triples)
            # save ent_node_to_chunk_ids
            with open(entity_node_to_chunk_ids_save_path, "w") as f:
                json.dump({
                    str(k): list(v) for k, v in self.ent_node_to_chunk_ids.items()
                }, f)
    ...
    def add_fact_edges(self, chunk_ids: List[str], chunk_triples: List[Tuple]):
        ...
        for chunk_key, triples in tqdm(zip(chunk_ids, chunk_triples), desc="Adding triples to graph", total=len(chunk_ids)):
    ...
    def prepare_retrieval_objects(self):
        ...
        loguru.logger.info("Preparing for fast retrieval.")

        loguru.logger.info("Loading keys.")
        self.query_to_embedding: Dict = {'triple': {}, 'passage': {}, 'entity': {}}
        ...
        loguru.logger.info("Loading embeddings.")
        ...
        proc_triples_to_docs_save_path = os.path.join(self.working_dir, "proc_triples_to_docs.json")
        if os.path.isfile(proc_triples_to_docs_save_path):
            with open(proc_triples_to_docs_save_path, 'r') as f:
                self.proc_triples_to_docs = json.load(f)
                self.proc_triples_to_docs = {k: set(v) for k, v in self.proc_triples_to_docs.items()}
            loguru.logger.info(f"Loading proc_triples_to_docs from {proc_triples_to_docs_save_path}")
        else:
            self.proc_triples_to_docs = {}

            for doc in tqdm(all_openie_info, desc="Processing OpenIE results", total=len(all_openie_info)):
            ...
            with open(proc_triples_to_docs_save_path, 'w') as f:
                json.dump({
                    k: list(v) for k, v in self.proc_triples_to_docs.items()
                }, f)


        if self.ent_node_to_chunk_ids is None:
            loguru.logger.info("setting up the passage node to chunk ids.")
            ner_results_dict, triple_results_dict = reformat_openie_results(all_openie_info)
