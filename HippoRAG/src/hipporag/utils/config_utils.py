...
class BaseConfig:
    ...
    embedding_model_device: str = field(
        default="cuda",
        metadata={"help": "Device for local embedding model."}
    )
    ...
    web_retrieval: bool = field(
        default=False,
        metadata={"help": "Whether to perform web retrieval or not."}
    )
    web_retrieval_url: str = field(
        default="http://127.0.0.1:18009/batch_web_search/",
        metadata={"help": "URL for web retrieval."}
    )
    ...
    language: str = field(
        default="EN",
        metadata={"help": "Language of the dataset to use. Currently only supports English."}
    )
    ...
    kg_file: Optional[str] = field(
        default=None,
        metadata={"help": "Path to the knowledge graph file."}
    )
