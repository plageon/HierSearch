...
class OpenIE:
    def __init__(self, llm_model: CacheOpenAI):
        ...
        self.openie_lang = os.getenv("OPENIE_LANG", "en")
        loguru.logger.info(f"OpenIE language: {self.openie_lang}")

    def ner(self, chunk_key: str, passage: str) -> NerRawOutput:
        # PREPROCESSING

        if self.openie_lang == "en":
            ner_input_message = self.prompt_template_manager.render(name='ner', passage=passage)
        elif self.openie_lang == "zh":
            ner_input_message = self.prompt_template_manager.render(name='ner_zh', passage=passage)
        else:
            raise ValueError(f"Unsupported language: {self.openie_lang}")
        raw_response = ""
        metadata = {}
        try:
            # LLM INFERENCE
    ...
    def batch_openie(self, chunks: Dict[str, ChunkInfo]) -> Tuple[Dict[str, NerRawOutput], Dict[str, TripleRawOutput]]:
        ...
        # sort chunk passages by chunk_key using ordered dict
        sorted_chunk_keys = sorted(chunks.keys())
        chunk_passages = OrderedDict()
        for chunk_key in sorted_chunk_keys:
            chunk_passages[chunk_key] = chunks[chunk_key]["content"]
