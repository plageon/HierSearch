...
def cache_response(func):
    ...
    def wrapper(self, *args, **kwargs):
        ...
        # loguru.logger.info(f"key_hash: {key_hash}, key_str: {key_str}")
...
class CacheOpenAI(BaseLLM):
    ...
    def __init__(self, cache_dir, global_config, cache_filename: str = None,
        ...
        if self.global_config.azure_endpoint is None:
            self.openai_client = OpenAI(base_url=self.llm_base_url, http_client=client, max_retries=self.max_retries, api_key=os.getenv("OPENAI_API_KEY", ""))
        else:
    ...
    def infer(
        ...
        **kwargs
        ...
        # import loguru
        # loguru.logger.info(f"Calling OpenAI GPT API with:\n{params}")
        ...
        # loguru.logger.info(f"response: {response.choices[0].message.content}")
